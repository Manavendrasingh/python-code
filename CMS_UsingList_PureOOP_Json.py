...
class Customer:
    ...
    @staticmethod
    def saveDatainFile():
        listInJson=json.dumps(Customer.listCus,default=Customer.convertCustomerIntoDict)
        fs=open("cusMgtjson.txt",'w')
        fs.write(listInJson)
        fs.close()
    @staticmethod
    def convertDictIntoCustomerAuto(dict1):
        cus = Customer()
    ...
    def loadDataFromFile():
        ...
        listinjson=fs.read()
        Customer.listCus=json.loads(listinjson,object_hook=Customer.convertDictIntoCustomerAuto)
